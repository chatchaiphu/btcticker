    ...
    for x in range(0, num_retries):
        ...
        if config["ticker"]["exchange"] == "default":
            ...
            if connectfail == True:
                ...
                other[
                    "market_cap_rank"
                ] = 0  # For non-default the Rank does not show in the API, so leave blank
...
def updateDisplay(config, other):
    ...
    for idx in range(0, len(others)):
        ...
        if os.path.isfile(tokenfilename):
            ...
            tokenimage = Image.open(
                requests.get(
                    rawimage["image"]["large"], headers=headers, stream=True
                ).raw
            ).convert("RGBA")
            resize = 100, 100
        ...
        if True:
            ...
            writewrappedlines(
                image,
                pricestring,
                FONT_PRICE_SIZE - fontreduce,
                EPD_PRICE_X,
                EPD_PRICE_Y + EPD_OFFSET_Y,
                8,
                15,
                font_info_name,
            )
            ...
            if "showrank" in config["display"] and config["display"]["showrank"]:
                ...
                if EPD_COLOR_NUM == 2:
                    drawtextalign(
                        ...
                        whichcoin[0:9]
                        + "["
                        + str("%d" % other["market_cap_rank"])
                        + "]",
                        EPD_NAME_X,
                        EPD_NAME_Y + EPD_OFFSET_Y,
                        EPD_NAME_W,
                        font=font_tail,
                        fill=0,
                        align=EPD_NAME_A,
    ...
    if (
        ...
        or config["display"]["orientation"] == 180
    ):
        image = image.rotate(180, expand=True)
        image2 = image2.rotate(180, expand=True)
...
def setupdisplay(config):
    ...
    if EPD_DISP_LAYOUT == 3 or EPD_DISP_LAYOUT == 4:
        if (
            ...
            and config["display"]["orientation"] != 180
        ):
            config["display"]["orientation"] = 0
    else:
        if (
            config["display"]["orientation"] != 90
            and config["display"]["orientation"] != 270
        ):
            config["display"]["orientation"] = 270
    ...
    font_date = ImageFont.truetype(
        os.path.join(fontdir, "whitrabt.ttf"), FONT_DATE_SIZE
    )
    font_tail = ImageFont.truetype(
        os.path.join(fontdir, "whitrabt.ttf"), FONT_TAIL_SIZE
    )
